...
/**
 *  \file       htdocs/custom/projecttaskspro/projet_tab_delete_tasks.php
 *  \ingroup    project
 *  \brief      List all tasks of a project and delete them
 */
...
$deletetasks = new ProjectTasksPro($db);
...
if ($id > 0 || !empty($ref)) {
    ...
    if ($action == 'delete') {
        ...
        foreach ($toselect as $id) {
            $sql = 'DELETE FROM ' . MAIN_DB_PREFIX . 'projet_task WHERE rowid = ' . $id . '';
            $resql = $db->query($sql);
            ...
            $sql = 'DELETE FROM ' . MAIN_DB_PREFIX . 'projet_task_extrafields WHERE fk_object = ' . $id . '';
            $resql = $db->query($sql);
    ...
    $tab = (GETPOSTISSET('tab') ? GETPOST('tab') : 'deleteprojecttasks');
...
if ($id > 0 || !empty($ref)) {
    ...
    if (count($tasksarray) > 0) {
        ...
        $j = 0;
        $level = 0;
        $nboftaskshown = $deletetasks->cptProjectLinesa($j, 0, $arrayofselected, $tasksarray, $level, true, 0, $tasksrole, $object->id, 1, $object->id, $filterprogresscalc, ($object->usage_bill_time ? 1 : 0), $arrayfields);
    } else {
        $colspan = 10;
        if ($object->usage_bill_time) {
            $colspan += 2;
    ...
    if ($user->rights->projet->creer) {
        if ($userWrite > 0) {
            ...
            print '<a id="po" class="butAction' . ($conf->use_javascript_ajax ? ' reposition' : '') . '" >' . $langs->trans("ToDelete") . '</a>';
            ...
            print '<script type="text/javascript">
                   ...
                   {
                      var pourl = "' . $_SERVER['PHP_SELF'] . '?id=' . $object->id . '&action=delete&det="+darr;
